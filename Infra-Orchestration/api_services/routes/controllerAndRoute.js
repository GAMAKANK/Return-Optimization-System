...
router.post('/api/mock-full-data', async (req, res) => {
    try {
        // 1. Create a Product
        const productData = {
            name: `Mock Product ${Date.now()}`,
            sku: `SKU-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
            category: 'Electronics',
            price: (Math.random() * 1000).toFixed(2),
            image: 'https://example.com/mock-product.jpg',
            description: 'A mock product created for testing purposes.'
        };
        ...
        const warehouseData = {
            ...
            location: {
                longitude: Math.random() * 180 - 90, // Random longitude
                latitude: Math.random() * 360 - 180 // Random latitude
            },
...
router.post('/inventory/assign', async (req, res) => {
  console.log('Assigning to shelf with data:', req.body);
  try {
    const { sku, batchId, quantityToAdd } = req.body;

    if (!sku || !batchId || !quantityToAdd) {
      return res.status(400).json({ error: 'SKU, batchId, and quantityToAdd are required.' });
    }

    console.log('Received data:', { sku, batchId, quantityToAdd });
    ...
    // Check if shelf already exists for this product and batch
    const existingShelf = await Shelf.findOne({productId:productId, batchId: batch_id});
    ...
    if (existingShelf) {
      ...
      // Find associated warehouse
      const inventoryLink = await WarehouseInventory.findOne({ shelfId: existingShelf._id });
      const warehouse = inventoryLink
        ? await Warehouse.findById(inventoryLink.warehouseId)
        : null;
    ...
    // Find an empty shelf
    const emptyShelf = await Shelf.findOne({ productId: null, batchId: null, quantity: 0 });

    if (!emptyShelf) {
      // Create a new shelf if no empty shelf is available
      const newShelf = new Shelf({
        productId: productId,
        batchId: batch_id,
        shelfNumber: `SHELF-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
        quantity: quantityToAdd
      });
      await newShelf.save();

      // Find or create warehouse inventory entry
      const warehouse = await Warehouse.findOne(); // Get first warehouse or modify as needed
      if (warehouse) {
        const newInventory = new WarehouseInventory({
      ...
      return res.status(201).json({
        message: 'New shelf created and product placed successfully',
        shelf: newShelf,
        warehouse
      });
    }
    ...
    emptyShelf.batchId = batch_id;
    emptyShelf.quantity = quantityToAdd;
    await emptyShelf.save();

    // Find associated warehouse
    const inventoryLink = await WarehouseInventory.findOne({ shelfId: emptyShelf._id });
    const warehouse = inventoryLink
      ? await Warehouse.findById(inventoryLink.warehouseId)
      : null;

    return res.status(201).json({
      message: 'Shelf assigned and product placed successfully',
      shelf: emptyShelf,
      warehouse
    });
